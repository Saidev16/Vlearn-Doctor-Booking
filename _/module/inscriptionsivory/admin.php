<?php
	   ...
	   <!--
	   		window.location.replace('gestion_inscription_ivory.php?code_inscription=<?=$pci?>');
	   -->
	   </script>
	   	<?php																   }
 	  ...
 	  $sql="SELECT c.titre, i.code_cours, count(i.code_inscription) as inscription from 
 		tbl_cours as c, tbl_inscription_cours_ivory as i, tbl_etudiant_ivory as e
  ...
  <tr>
	...
	<td width="22%">
	 <table border="0" align="right" width="20%" cellpadding="10" cellspacing="4" id="link" >
	    <tr>
		 <td valign="top" align="center"><a href="gestion_inscription_ivory.php?full_add=oui"><div class="ajouter"></div>Add Testing Course</a></td>
		 <td valign="top" align="center"><a href="gestion_inscription_ivory.php?full_addc=oui"><div class="ajouter"></div>Add Course</a></td>
		 <td valign="top" align="center"><a href="gestion_inscription_ivory.php?full_addch=oui"><div class="ajouter"></div>Add Challenge Course</a></td>
		 <td valign="top" align="center"><a href="gestion_inscription_ivory.php?full_addT=oui"><div class="ajouter"></div>Add Transfer Course</a></td
		></tr>
	</table>
    </td> 
</table>
...
<div class="container_search"> 
 ...
 <?php 
	  ...
	  $ns=$row['session'].' '.$an=$row['annee_academique'];
 	  ...
 	  ?>
 	  <option  value="<?=$is?>" <?=($idSession==$is) ? $selected : ''?>><?=$cc?></option>
	  <?php
	  }
	  ?>
  </select>
<!-- <select name="type" class="search" >
 <?php 
      $sql2="SELECT id, title FROM $tbl_groupe";
 	  $req2=@mysql_query($sql2) or die("erreur lors de la sélection des groupes");
 	  while ($row = mysql_fetch_assoc($req2)){
	  $id=$row['id'];
	  $title=$row['title'];
 	  ?>
  ...
  <select name="groupe" class="search" title="Recherchez par le nom du groupe du cours">
    ...
    <select name="code_inscription" class="search" onchange="javascript:document.adminMenu.submit()" title="S&eacute;l&eacute;ctionnez un &eacute;tudiant pour voir ses cours">
		...
		from tbl_etudiant_ivory as e, tbl_inscription_cours_ivory as i
  ...
  <tr height="16px">
	 ...
	 <td align="left"><a href="gestion_inscription_ivory.php?code_cours=<?=$cc?>" title="Cliquez pour voir les &eacute;tudiants inscrits dans ce cours">&nbsp;<?=$cc?></a></td>
 	 <td align="left">&nbsp;<?=stripslashes(trim($row["titre"]))?></td>
	 <?php
	/* $inscri="select count(*) as nbre from tbl_inscription_cours_ivory_ivory where code_cours='$cc' and idSession='$idSession'";*/
	$inscri="select count(*) as nbre  from tbl_inscription_cours_ivory as c, tbl_note_ivory as n 
