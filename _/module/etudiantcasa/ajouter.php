...
if ($message == 0 ) {
 $sqlzin="SELECT max(`code_inscription`) as max FROM tbl_etudiant_casa";
$sqlzin1 =mysql_query($sqlzin) or die("erreur lors de traitement de F*");

$ab= mysql_fetch_assoc($sqlzin1);
$cu= $ab["max"];
$code_inscription= $cu+1;
  $sql_insert="INSERT INTO tbl_etudiant_casa (`prefixe` ,code_inscription,`nom` , `prenom` , `date_naissance` , `nationalite` , `adresse` , `sexe` , `code_bac` , `serie_bac` ,
...
VALUES ('CB','$code_inscription','$nom', '$prenom', '$date_naissance', '$nationalite', '$adresse', '$sexe', '$code_bac', '$serie_bac', '$date_insc', '$tel', '$mail', '$annee', '$idSession', '$filiere', '$lieu_naissance', '$cin', '$cinq_photo', '$original_bac', '$copie_bac', '$english_bac', '$trois_lettre', '$trois_enveloppe', '$buletin_note',  '$reglement', '$copie_cin', '$extrait', '$note_etudesoc', '$note_fr', '$note_sciences', '$note_math', '$aquise', '$nom', '$pass', 1, '$groupe', '$activite', '$annee_inscription', '$niveau', '$ville', '$elearning', '$piimt', '$aul', '$umt', '$type','$parentname', '$parentphone', '$parentemail', '$parentname', '$pass');";
...
/*$sqlfinance="insert into tbl_finance (`prefixe` ,code_inscription,frais_etude) values('MOR','$code_inscription',$tuition)";
 ...
 $sqlfinance="insert into tbl_finance_casa (`prefixe` ,code_inscription) values('CB','$code_inscription')";
 @mysql_query($sqlfinance)or die ("Error save finance");
          ...
          if ($type !='Prospect') { ?>
           ...
           ?>
 			...
 			<!--
                       window.location.replace('/module/api_testing/create_user.php?code_inscription=<?php echo $code_inscription; ?>&prefixe=CB');
 			//-->
 			</script>
               <?php
                   }
      ...
      ?>
        <script type="text/javascript" language="JavaScript1.2">
            <!--
            window.location.replace('gestion_des_etudiants_casa.php');
            //-->
        </script>
      <?php
    }
	...
	<td width="20%">
	  <table border="0" align="right" width="0%" cellpadding="10" cellspacing="4" id="link" >
	    <tr>
		  ...
		  <td valign="top" align="center">
		   <a href="gestion_des_etudiants_casa.php"><div class="cancel"></div>Cancel</a>
		  </td>
		</tr>
	  </table>
	</td>
...
<style media="screen">
  ...
  <form method="post" action="gestion_des_etudiants_casa.php?new=oui" name="f_ajout" >
