...
$idSession = $_POST['idSession'];
 $sqlzin="SELECT max(`code_inscription`) as max FROM tbl_etudiant_benin";
$sqlzin1 =mysql_query($sqlzin) or die("erreur lors de traitement de F*");

$ab= mysql_fetch_assoc($sqlzin1);
$cu= $ab["max"];
$code_inscription= $cu+1;
 $sql_insert="INSERT INTO tbl_etudiant_benin (`prefixe` ,code_inscription,`nom` , `prenom` , `date_naissance` , `nationalite` , `adresse` , `sexe` , `code_bac` , `serie_bac` ,
...
VALUES ('BN','$code_inscription','$nom', '$prenom', '$date_naissance', '$nationalite', '$adresse', '$sexe', '$code_bac', '$serie_bac', '$date_insc', '$tel', '$mail', '$annee', '$idSession', '$filiere', '$lieu_naissance', '$cin', '$cinq_photo', '$original_bac', '$copie_bac', '$english_bac', '$trois_lettre', '$trois_enveloppe', '$buletin_note',  '$reglement', '$copie_cin', '$extrait', '$note_etudesoc', '$note_fr', '$note_sciences', '$note_math', '$aquise', '$nom', '$pass', 1, '$groupe', '$activite', '$annee_inscription', '$niveau', '$ville', '$elearning', '$piimt', '$aul', '$umt');";
	  @mysql_query($sql_insert)or die ("Error save student");

	  $a1 ="insert into tbl_finance_benin (`prefixe` ,code_inscription) values ('BN','$code_inscription') ";
      $a1 = mysql_query($a1)or die ('Erreur:: selectionCours');
	...
	if($groupe==2)
	{
		    ...
		    if ($code_cours != '' )
			{
				//$type_cours = str_replace('_', ' ', $_POST['special_price'.$i]);
		      	$sql = "SELECT COUNT(*) AS nbr FROM tbl_inscription_cours_benin
		...
		if ($nbr==0)
		{
			...
			/*$sql = "INSERT INTO tbl_inscription_cours_benin (`code_cours`, `code_inscription`, `date_inscription`, `idSession`,`type_cours_id`, `section`)
			...
			$sql = "insert into tbl_note_benin(code_inscription, code_cours,code_cours_testing 	, idSession, archive, section,letter_grade)
...

{
	...
	$j++;
           ...
           ?>
 			...
 			<!--
                       window.location.replace('/module/api_testing/create_user.php?code_inscription=<?php echo $code_inscription; ?>&prefixe=BN');
 			//-->
 			</script>
               <?php
                   }
...
<table border="0" width="100%" cellpadding="0" cellspacing="0" align="center" class="haut_table">
	...
	<td width="20%">
	  <table border="0" align="right" width="0%" cellpadding="10" cellspacing="4" id="link" >
	    <tr>
		  ...
		  <td valign="top" align="center">
		   <a href="gestion_des_etudiants_benin.php"><div class="cancel"></div>Cancel</a>
		  </td>
		</tr>
	  </table>
	</td>
  ...
  <form method="post" action="gestion_des_etudiants_benin.php?new=oui" name="f_ajout" >
