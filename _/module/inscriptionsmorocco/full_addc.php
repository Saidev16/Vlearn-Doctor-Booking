...
<?php
		...
		$sql = "SELECT COUNT(*) AS nbr FROM tbl_inscription_cours_morocco WHERE code_inscription='$code_inscription' 
		...
		if ($nbr==0){
			...
			$sql2014 = "SELECT code_cours_testing,code_cours_duplicata FROM tbl_cours WHERE code_cours = '$code_cours'";
			$res2014 = mysql_query($sql2014);
			$row = mysql_fetch_assoc($res2014);
			$code_cours_testing = $row['code_cours_testing'];
			$code_cours_duplicata = $row['code_cours_duplicata'];
...
<table border="0" width="100%" cellpadding="0" cellspacing="0" align="center" class="haut_table">
  <tr>
    ...
    <td width="78%" class="titre">&nbsp;Course Registration <span class="task">[Add]</span> 
    </td>
	<td width="22%">
	  <table border="0" align="right" width="0%" cellpadding="10" cellspacing="4" id="link" >
	    <tr>
		  ...
		  <td valign="top" align="center">
		   <a href="gestion_inscription_morocco.php" ><div class="cancel"></div>Cancel</a>
		  </td>
		</tr>
	  </table>
	</td> 
 ...
 <form method="post" action="gestion_inscription_morocco.php?full_addc=oui" name="f_ajout">
		  ...
		  <?php 
			$sql5="SELECT  code_cours, titre ,nbr_credit FROM $tbl_cours  GROUP BY  code_cours";
			...
			$cr=$row['nbr_credit'];
		  ?>
		  <option value="<?=$cc?>"><?=$cc?>:<?=$tc?>:credit:<?=$cr?></option>
		  <?php
		  }
		  ?>
		  </select>
