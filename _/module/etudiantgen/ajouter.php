...
$sqlzin="SELECT max(`code_inscription`) as max FROM tbl_etudiant_morocco";
...
$code_inscription= $cu+1;
 ...
 $sql_insert="INSERT INTO tbl_etudiant_morocco (`prefixe` ,code_inscription,`nom` , `prenom` , `date_naissance` , `nationalite` , `adresse` , `sexe` , `code_bac` , `serie_bac` ,
...
VALUES ('MOR','$code_inscription','$nom', '$prenom', '$date_naissance', '$nationalite', '$adresse', '$sexe', '$code_bac', '$serie_bac', '$date_insc', '$tel', '$mail', '$annee', '$semestre', '$filiere', '$lieu_naissance', '$cin', '$cinq_photo', '$original_bac', '$copie_bac', '$english_bac', '$trois_lettre', '$trois_enveloppe', '$buletin_note',  '$reglement', '$copie_cin', '$extrait', '$note_etudesoc', '$note_fr', '$note_sciences', '$note_math', '$aquise', '$nom', '$pass', 1, '$groupe', '$activite', '$annee_inscription', '$niveau', '$ville', '$elearning', '$piimt', '$aul', '$umt');"; 
	  @mysql_query($sql_insert)or die ("Error save studentPSI");
  $sql_insert1="INSERT INTO tbl_etudiant_usa (`prefixe` ,code_inscription,`nom` , `prenom` , `date_naissance` , `nationalite` , `adresse` , `sexe` , `code_bac` , `serie_bac` ,
`date_inscription` , `tel` , `email` , `annee` , `semestre` , `filiere` , `lieu_naissance` , `cin` , `cinq_photo` , `original_bac` ,
`copie_bac` , `english_bac` , `trois_lettre` , `trois_enveloppe` , `buletin` , `reglement` , `copie_cin` ,
`extrait_naissance` , `test_etudesoc` , `test_fr` , `test_sciences` , `test_math` , `aquise_academique` , `login` , `mot_pass` ,
`acces` , `groupe` , `activite` , `annee_inscription`, `niveau` , `ville` , `elearning`, `piimt`, `aul`, `umt`
...
VALUES ('MOR','$code_inscription','$nom', '$prenom', '$date_naissance', '$nationalite', '$adresse', '$sexe', '$code_bac', '$serie_bac', '$date_insc', '$tel', '$mail', '$annee', '$semestre', '$filiere', '$lieu_naissance', '$cin', '$cinq_photo', '$original_bac', '$copie_bac', '$english_bac', '$trois_lettre', '$trois_enveloppe', '$buletin_note',  '$reglement', '$copie_cin', '$extrait', '$note_etudesoc', '$note_fr', '$note_sciences', '$note_math', '$aquise', '$nom', '$pass', 1, '$groupe', '$activite', '$annee_inscription', '$niveau', '$ville', '$elearning', '$piimt', '$aul', '$umt');"; 
	  @mysql_query($sql_insert1)or die ("Error save studentPSI");
	  
	$sql14 = "SELECT code_inscription FROM tbl_etudiant_burkina 
	...
	if($groupe==2)
	{
		 	...
		 	$a ="select code_cours from $tbl_cours where  diff=2 or diff=1";
		...
		if ($nbr==0)
		{
			...
			/*$sql = "INSERT INTO tbl_inscription_cours_burkina (`code_cours`, `code_inscription`, `date_inscription`, `idSession`,`type_cours_id`, `section`)
					VALUES ('$code_cours', '$code_inscription', '$date', '$idSession ', '$type_cours', '$niveau' )";
 	         		@mysql_query($sql)or die ('Erreur :: inscription ');
			// creation de la fiche de notes
			$sql = "insert into tbl_note_burkina(code_inscription, code_cours, idSession, archive, section,letter_grade) 
					value('$code_inscription', '$code_cours', '$idSession', 0, '$niveau','X')"; 
...

{
		 	...
		 	$a ="select code_cours from $tbl_cours where  diff=3 or diff=1";
		 	$aa = mysql_query($a)or die ('Erreur:: selectionCours');
			$transcript= array ();
			$j=0;
...
<table border="0" width="100%" cellpadding="0" cellspacing="0" align="center" class="haut_table">
	...
	<td width="20%">
	  <table border="0" align="right" width="0%" cellpadding="10" cellspacing="4" id="link" >
	    <tr>
		  ...
		  <td valign="top" align="center">
		   <a href="gestion_des_etudiants_morocco.php"><div class="cancel"></div>Cancel</a>
		  </td>
		</tr>
	  </table>
	</td> 
  ...
  <form method="post" action="gestion_des_etudiants_morocco.php?new=oui" name="f_ajout" >
