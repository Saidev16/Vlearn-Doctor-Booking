...
$sqlzin="SELECT max(`code_inscription`) as max FROM tbl_etudiant_algeria";
...
$code_inscription= $cu+1;
 ...
 $sql_insert="INSERT INTO tbl_etudiant_algeria (`prefixe` ,code_inscription,`nom` , `prenom` , `date_naissance` , `nationalite` , `adresse` , `sexe` , `code_bac` , `serie_bac` ,
...
VALUES ('AG','$code_inscription','$nom', '$prenom', '$date_naissance', '$nationalite', '$adresse', '$sexe', '$code_bac', '$serie_bac', '$date_insc', '$tel', '$mail', '$annee', '$semestre', '$filiere', '$lieu_naissance', '$cin', '$cinq_photo', '$original_bac', '$copie_bac', '$english_bac', '$trois_lettre', '$trois_enveloppe', '$buletin_note',  '$reglement', '$copie_cin', '$extrait', '$note_etudesoc', '$note_fr', '$note_sciences', '$note_math', '$aquise', '$nom', '$pass', 1, '$groupe', '$activite', '$annee_inscription', '$niveau', '$ville', '$elearning', '$piimt', '$aul', '$umt');"; 
	  @mysql_query($sql_insert)or die ("Error save studentPSI");

	  $sqlfinance="insert into tbl_finance_algeria (`prefixe` ,code_inscription) values('AG','$code_inscription')";
 @mysql_query($sqlfinance)or die ("Error save finance");
	  
	$sql14 = "SELECT code_inscription FROM tbl_etudiant_algeria 
	...
	if($groupe==2)
	{
		 	...
		 	$a ="select code_cours,code_cours_testing from $tbl_cours where  diff=2 or diff=1";
			...
			$j=0;
		 	...
		 	while ($ligne= mysql_fetch_assoc($aa))
		 	{
					...
					//collecter les cours dans tableau
					$transcript[$j]= $code_cours;
					
		    if ($code_cours != '' ) 
			{
				//$type_cours = str_replace('_', ' ', $_POST['special_price'.$i]);
		      	$sql = "SELECT COUNT(*) AS nbr FROM tbl_inscription_cours_algeria 
		...
		if ($nbr==0)
		{
			...
			/*$sql = "INSERT INTO tbl_inscription_cours_algeria (`code_cours`,code_cours_testing, `code_inscription`, `date_inscription`, `idSession`,`type_cours_id`, `section`)
					VALUES ('$code_cours','$code_cours_testing', '$code_inscription', '$date', '$idSession ', '$type_cours', '$niveau' )";
 	         		@mysql_query($sql)or die ('Erreur :: inscription ');
			// creation de la fiche de notes
			$sql = "insert into tbl_note_algeria(code_inscription,code_cours_testing, code_cours, idSession, archive, section,letter_grade) 
					value('$code_inscription','$code_cours_testing', '$code_cours', '$idSession', 0, '$niveau','X')"; 
...

{
		 	...
		 	$a ="select code_cours,code_cours_testing from $tbl_cours where  diff=3 or diff=1";
		 	$aa = mysql_query($a)or die ('Erreur:: selectionCours');
			$transcript= array ();
			$j=0;
		...
		if ($nbr==0)
		{
			...
			/*$sql = "INSERT INTO tbl_inscription_cours_algeria (`code_cours`,code_cours_testing, `code_inscription`, `date_inscription`, `idSession`,`type_cours_id`, `section`)
					VALUES ('$code_cours', '$code_cours_testing','$code_inscription', '$date', '$idSession ', '$type_cours', '$niveau' )";
 	         		@mysql_query($sql)or die ('Erreur :: inscription ');
			// creation de la fiche de notes
			$sql = "insert into tbl_note_algeria(code_inscription, code_cours,code_cours_testing, idSession, archive, section,letter_grade) 
					value('$code_inscription', '$code_cours','$code_cours_testing', '$idSession', 0, '$niveau','X')"; 
			@mysql_query($sql) or die ('erreur lors de creation de la fiche  de notes');*/
...
<table border="0" width="100%" cellpadding="0" cellspacing="0" align="center" class="haut_table">
  ...
  <form method="post" action="gestion_des_etudiants_algeria.php?new=oui" name="f_ajout" >
...
<option value="Zimbabwe">Zimbabwe</option>
         ...
         <tr>
           ...
           <tr>
             <td><label for="groupe"><?php echo 'Section:';?> </label></td>
			  <td>
					...
					while($row=mysql_fetch_assoc($req)){
					?>
				  <option value="<?=$row['id']?>">&nbsp;<?=$_SESSION['admin_lang']== 'eng' ? $row['title_eng'] : $row['title']?></option>
					<?php }?>
				  </select>			  </td>
				   <td></td>
