...
<?php	
	...
	if (isset($_POST['code_inscription'])){
		...
		$sql="UPDATE tbl_finance_paiement_morocco SET designation = '$designation', 
		...
		$sql="select sum(somme) as nbre from tbl_finance_paiement_morocco  where code_inscription = '$code_inscription'";
	     ...
	     $payee=$ab["nbre"];
			...
			//-->
			 ...
			 $sql="SELECT e.nom, e.prenom ,e.prefixe, p.* 
						 FROM tbl_etudiant_morocco AS e, tbl_finance_paiement_morocco AS p
		         ...
		         echo   $d = substr($row['date_paiement'], 8,2);//$row['date_paiement'];
				  $somme= $row['somme'];
		          $recu= $row['recu'];
...
<table border="0" width="100%" cellpadding="0" cellspacing="0" align="center" class="haut_table">
	...
	<td width="22%">
	  <table border="0" align="right" width="0%" cellpadding="10" cellspacing="4" id="link" >
	    <tr>
		  ...
		  <td valign="top" align="center">
		   <a href="gestion_finance_morocco.php?detail=<?php echo $code_inscription ?>" ><div class="cancel"></div>Cancel</a>
		  </td>
		</tr>
	  </table>
	</td> 
 ...
 <form method="post" action="gestion_finance_morocco.php?modif_paiement=oui" name="f_ajout" >
         ...
         <td valign="top" width="100%">
	       ...
	       <table border="0" cellpadding="0" cellspacing="2" width="100%" class="cellule_table"> 
		    <tr>
			 ...
			 <tr>
		  		<td class="gras"><label for="recu">Receipt Number : </label></td>
				...
				<select name="type_paiement" id="type_paiement"   class="input" />
					...
					<option value="0" <?=($type_paiement==0) ? $selected : '' ?>>Cash</option>
					<option value="1" <?=($type_paiement==1) ? $selected : '' ?>>Check</option>
					<option value="2" <?=($type_paiement==2) ? $selected : '' ?>>Deposits </option>
					<option value="3" <?=($type_paiement==3) ? $selected : '' ?>>Payapl </option>	
					<option value="4" <?=($type_paiement==4) ? $selected : '' ?>>Bank Wire </option>	
					<option value="5" <?=($type_paiement==5) ? $selected : '' ?>>Credit Card </option>			
				</select>
				</td>
		   </tr>
			  <tr>
		  ...
		  // for ($i=date('Y')-2; $i>1996,$i<2022; $i++)
		   	 for ($i=date('Y')-4; $i>1996,$i<2022; $i++)
		   	 	{?>
