		   ...
		   <a href="#" 
		     ...
		     onclick="javascript:if(document.adminMenu.boxchecked.value==0)
...

				   {
					 ...
					 chemin='gestion_finance_morocco.php?modifier='+chemin;
...
<table width="100%" align="center" cellspacing="1"  class="adminlist" style="margin-top:2px">
  ...
  <th width="20">&nbsp;</th>
	  ...
	  <th width="84">Program Tuition</th>
	    <th width="84">Registration Fee</th>
      <th width="114">Courses Fees</th>     
       <th width="84">Challenge Fees</th>
        <th width="84">Scholarship</th>
    ...
    <?php
     ...
     $sql = "SELECT DISTINCT e.code_inscription, CONCAT(e.nom,' ', e.prenom) AS name, f.*,e.tel ,e.ville,f.prefixe
		 FROM tbl_etudiant_morocco AS e, tbl_finance_morocco AS f
	 ...
	 $nbre=$row['nbre'];
	  $sum="select sum(somme) as somme from  tbl_finance_paiement_morocco where code_inscription='$ci' ";                                   
       $req12=@mysql_query($sum) or die('erreur de selection des paiements');
          $row12 = mysql_fetch_assoc($req12);
       $somme = $row12["somme"];


	  $reste = ((int)$row["frais_etude"]+((int)$row['frais_inscription']*100)+(int)$row["challenge"]+(int)$row["Reinstatement"]+(int)$row["Returned"]+((int)$row["Replacement"]*$nbre)+(int)$row["Diploma"]+(int)$row["uspostal"]+(int)$row["fedex"])-($somme+$bourse);
 ...
 <td><input type="radio" name="id" value="<?=$ci?>" onclick="document.adminMenu.boxchecked.value='<?=$ci?>'" /></td>
      <td align="left">&nbsp;<a href="gestion_finance_morocco.php?detail=<?=$ci;?>" >
        <?=$name?>
      </a></td>
	   <td>&nbsp;
          <?=number_format($row["frais_etude"])?></td>
           <td>&nbsp;
          <?=number_format($row["Registration"])?></td>
	
      <td>&nbsp;
          <?=number_format($row["frais_inscription"]*100)?></td>
     
           <td>&nbsp;
          <?=number_format($row["challenge"])?></td>
            <td>&nbsp;
...
//$total=$row["frais_inscription"]+$row["frais_etude"]+$row["challenge"];
//$reste=$total-$somme;
