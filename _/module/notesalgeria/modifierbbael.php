...
$stage2=$_POST['stage2'];
$stage3=$_POST['stage3'];
$stage=($stage1+ $stage2+ $stage3)/3;
if ($stage <= 59 ){
 $letter_grade= 'F';
 $GPA=0;
 }
 
  if ( $stage <= 63  &&  $stage >= 60  ){
 $letter_grade= 'D-';
 $GPA=1;
 }
 
 if ( $stage <= 66  &&  $stage >= 64 ){
 $letter_grade= 'D';
 $GPA=1;
 }
 
 if ($stage <= 69  &&  $stage >= 67 ){
 $letter_grade= 'D+';
 $GPA=1;
 }
 
  if ( $stage <= 73  && $stage >= 70  ){
 $letter_grade= 'C-';
 $GPA=2;
 }
 
 if ( $stage <= 76  &&  $stage >= 74  ){
 $letter_grade= 'C';
 $GPA=2;
 }
 
 if ( $stage <= 79  && $stage >= 77  ){
 $letter_grade= 'C+';
 $GPA=2;
 }
 
 if (  $stage <= 83  &&  $stage >= 80  ){
 $letter_grade= 'B-';
 $GPA=3;
 }
 
 if (  $stage <= 86  &&  $stage >= 84  ){
 $letter_grade= 'B';
 $GPA=3;
 }
 
 if (  $stage <= 89  &&  $stage >= 87  ){
 $letter_grade= 'B+';
 $GPA=3;
 }
 
 if ( $final_grade <= 93  &&  $stage >= 90  ){
 $letter_grade= 'A-';
 $GPA=4;
 }
 
 if ( $stage <= 100  &&  $stage >= 94  ){
 $letter_grade= 'A';
 $GPA=4;
 }

$sql12="UPDATE $tbl_note SET 
			...
			`letter_grade` = '$letter_grade'
 			 ...
 			 WHERE `code_note` = '$id' ";  
			 $req12=mysql_query($sql12) or die("erreur lors de l'insertion des notes");
...
$some_coef=0;
  if(isset($_POST['transfer']) and $_POST['transfer']==1){
 	 ...
 	 $letter_grade= 'W';
		...
		if ( (!empty($mid_term))  &&  (!empty($final_exam)) &&  (!empty($participation)) )
		{
		// declare coeficient if 4 marks
		$coef_mid_term=35;
		
		$coef_final_grade=45;
		$coef_participation=20;
		// calcule final grade 
		$final_grade=(($mid_term*$coef_mid_term)  + ($participation*$coef_participation) + ($final_exam*$coef_final_grade))/ 100;
		 }
...
$aa="select code_inscription,section from tbl_note where code_note='$id' ";
$reqaa=mysql_query($aa) or die ("erreur lors de la selection du section");
$aaa=mysql_fetch_assoc($reqaa);
$sect=$aaa['section'];
$code=$aaa['code_inscription'];

$b="select count(*) as nbre from tbl_note where letter_grade!='' and letter_grade!='I' and section='$sect' and code_inscription='$code'";
$reqb=mysql_query($b) or die ("erreur lors de la selection du section");
$ct=mysql_fetch_assoc($reqb);
$nbre=$ct["nbre"];
if($sect=='bba' or $sect=='bscs' or $sect=='bsbe')
{
if($nbre*3==120 )
{$bba="update $tbl_etudiant set laureatbba=1 where code_inscription='$code'";
$reqbba=mysql_query($bba) or die ("erreur lors de la selection du section");} 
else {$bba="update $tbl_etudiant set laureatbba=0 where code_inscription='$code'";
$reqbba=mysql_query($bba) or die ("erreur lors de la selection du section");}}
else if ($sect=='mba' or $sect=='mm')
{
if($nbre*3==36 || $nbre*3==48)
{$mba="update $tbl_etudiant set laureatmba=1 where code_inscription='$code'";
$reqmba=mysql_query($mba) or die ("erreur lors de la selection du section");}

else
{$mba="update $tbl_etudiant set laureatmba=00 where code_inscription='$code'";
$reqmba=mysql_query($mba) or die ("erreur lors de la selection du section");} }
...
<table border="0" width="100%" cellpadding="0" cellspacing="0" align="center" class="haut_table">
 ...
 <form method="post"  action="gestion_notes.php?modifierbbael=oui" name="f_ajout">
    ...
    <input type="hidden" name="idSession" value="<?=$row['idSession']?>" />
		...
		<tr>
		  ...
		  <td><input type="text" name="final_exam" id="final_exam" value="<?=$row['final_exam']?>" class="input" size="7"  />
		   ...
		   <tr>
		  <td class="gras"><label for="fStart">State</label>
		  : </td>
 ...
 <form method="post"  action="gestion_notes.php?modifierbbael=oui" name="f_ajout">
    ...
    <input type="hidden" name="idSession" value="<?=$row['idSession']?>" />
		...
		<tr>
		  ...
		  <tr><td colspan="4" height="3px"></td></tr>
		  <tr>
		  <td width="25%" class="gras"><label for="stage1">Stage1:</label></td>
		  <td width="25%">
		  <input type="text" name="stage1" id="stage1" value="<?=$row['stage1']?>" class="input" size="7" />
		  </td>
		  <td width="25%"> </td>
		  <td width="25%"></td>
		  </tr>
		  <tr><td colspan="4" height="3px"></td></tr>
		   <tr>
		  <td class="gras"><label for="stage2">Stage2 :</label> </td>
		  <td><input type="text" name="stage2" id="stage2" value="<?=$row['stage2']?>" size="7" class="input" /></td>
		   <td>&nbsp;</td>
		   <td>&nbsp;</td>
		   </tr>
		  <tr><td colspan="4" height="3px"></td>
 		   ...
 		   <tr>
		  <td class="gras"><label for="stage3">Stage3 :</label> </td>
		  <td><input type="text" name="stage3" id="stage3" value="<?=$row['stage3']?>" size="7" class="input" /></td>
		   <td>&nbsp;</td>
		   <td>&nbsp;</td>
		   </tr>
		  <tr><td colspan="4" height="3px"></td>
