...
<div class="row">
  <?php if (isset($_GET['surveys_results_dba']) && isset($_GET['id'])): ?>
    ...
    <?php if (isset($_GET['code_prof']) && isset($_GET['annee']) && isset($_GET['periode'])){ ?>
      ...
      <h2 class="text-info">DBA Session: <?php  echo $periode.' '.$annee_per; ?></h2>
              ...
              <div class="panel panel-default">
                <div class="panel-heading">
                  <h4 class="panel-title text-info">
                    ...
                    $sql_new = "SELECT r.`id_affectation` , r.`id_reponse` , rep.`porcentage` as porcentage,count(r.`id_reponse`) as nb_porcentage FROM `tbl_sondage_affectation_dba` as a , `tbl_resultat_sondage` as r , `tbl_reponse` as rep , `tbl_aff_etudiant` as e , `tbl_etudiant` as et  WHERE  a.`id` = r.`id_affectation` and r.`id_reponse` = rep.`id` and a.`id` = $id_affectation and e.`code_inscription` = et.`code_inscription`group by rep.`porcentage`";
                    //var_dump($sql_new);
                    ...
                    $satisfaction = 0;
                    $porcentage1 = 0;
                    $sql_new = "SELECT count(distinct(r.code_etudiant)) as nb ,`id_question`,r.code_etudiant, r.`id_affectation` , r.`id_reponse` , rep.`porcentage` as porcentage FROM `tbl_sondage_affectation_dba` as a , `tbl_resultat_sondage` as r , `tbl_reponse` as rep , `tbl_aff_etudiant` as e , `tbl_etudiant` as et  WHERE  a.`id` = r.`id_affectation` and r.`id_reponse` = rep.`id` and a.`id` = $id_affectation and e.`code_inscription` = et.`code_inscription` and r.id_question  in (9,22,21,19,18,17,13) group by rep.`porcentage`,`code_etudiant`,`id_question`";
                    //var_dump($sql_new);
                    $req_new = @mysql_query($sql_new);
                    while ($row_new = mysql_fetch_array($req_new)) {
                    ...
                    $sql_new = "SELECT count(distinct(r.code_etudiant)) as nb ,`id_question`,r.code_etudiant, r.`id_affectation` , r.`id_reponse` , rep.`porcentage` as porcentage FROM `tbl_sondage_affectation_dba` as a , `tbl_resultat_sondage` as r , `tbl_reponse` as rep , `tbl_aff_etudiant` as e , `tbl_etudiant_eng` as et  WHERE  a.`id` = r.`id_affectation` and r.`id_reponse` = rep.`id` and a.`id` = $id_affectation and e.`code_inscription` = et.`code_inscription` and r.id_question  in (9,22,21,19,18,17,13) group by rep.`porcentage`,`code_etudiant`,`id_question`";
                    //var_dump($sql_new);
                    $req_new = @mysql_query($sql_new);
                    while ($row_new = mysql_fetch_array($req_new)) {
                      $satisfaction = $satisfaction + $row_new['porcentage'];
                    ...
                    // calcule
                    $sql_new = "SELECT count(distinct(r.code_etudiant)) as nb ,r.code_etudiant,r.`id_question`, r.`id_affectation` , r.`id_reponse` , rep.`porcentage` as porcentage FROM `tbl_sondage_affectation_dba` as a , `tbl_resultat_sondage` as r , `tbl_reponse` as rep , `tbl_aff_etudiant` as e , `tbl_etudiant` as et  WHERE  a.`id` = r.`id_affectation` and r.`id_reponse` = rep.`id` and a.`id` = $id_affectation and e.`code_inscription` = et.`code_inscription` and r.id_question in (1,2,3,4,5,6,7,8,15,16,23,24,25,26,27,31) group by rep.`porcentage` , r.`id_question`, r.`code_etudiant` order by `id_question`
                    	";
                    //var_dump($sql_new);
                    $req_new = @mysql_query($sql_new);
                    $satisfaction = 0;
                    ...
                    $sql_new = "SELECT count(distinct(r.code_etudiant)) as nb ,r.code_etudiant,r.`id_question`, r.`id_affectation` , r.`id_reponse` , rep.`porcentage` as porcentage FROM `tbl_sondage_affectation_dba` as a , `tbl_resultat_sondage` as r , `tbl_reponse` as rep , `tbl_aff_etudiant` as e , `tbl_etudiant_eng` as et  WHERE  a.`id` = r.`id_affectation` and r.`id_reponse` = rep.`id`
                    and a.`id` = $id_affectation and e.`code_inscription` = et.`code_inscription` and r.id_question in (1,2,3,4,5,6,7,8,15,16,23,24,25,26,27,31) group by rep.`porcentage` , r.`id_question`, r.`code_etudiant` order by `id_question`";
                    //var_dump($sql_new);
                    $req_new = @mysql_query($sql_new);
                    $satisfaction = 0;
                    while ($row_new = mysql_fetch_array($req_new)) {
                      ...
                      <?php }else{ ?>
                        ...
                        while ($row_periode = mysql_fetch_array($req_periode)) {
                         ...
                         //if ($periode == "fall" && $annee == 2019){
                          ...
                          `tbl_sondage_affectation_dba` as a ,
                          `tbl_resultat_sondage` as r ,
                          `tbl_reponse` as rep , `tbl_aff_etudiant` as e , `tbl_etudiant_eng` as et WHERE
                          a.`groupe` = 3 and
                          a.`niveau` = \"dba\" and
                          a.`annee` = $annee and
                          a.`periode` = \"$periode\" and
