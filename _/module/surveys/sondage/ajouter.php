<?php
	if (isset($_GET['new_sondage']) && $_GET['new_sondage'] == 'add') {
		...
		if ($message_titre_en == "1" || $message_session == "1" || $message_statut == "1"
			...
			$year = $_POST['year'];
			$date =date("j/n/Y");
			$sql_insert = "	INSERT INTO `tbl_sondage`(`titre_fr`,`titre_en`,`id_session`,`type`, `date_creation`,`statut`,`year`)
			 				VALUES (\"$titre_en\",\"$titre_en\",\"$session\",\"$type\",\"$date\",\"$statut\",\"$year\")";
			// var_dump($sql_insert);
			// die();
	...
	<form style="text-align: left;margin-top: 80px;" action="Surveys.php?new_sondage=add" class="form-horizontal" method="POST" >
		...
		<div style="margin-right: 175px;" class="form-group <?php if ((isset($_SESSION['message_session']) && $_SESSION['message_session'] !=  "") || (isset($_SESSION['message_session_unique']) && $_SESSION['message_session_unique'] !=  "")) { echo 'has-error';} ?>">
			...
			<div style="margin-right: 175px;" class="form-group <?php if (isset($_SESSION['message_type']) && $_SESSION['message_type'] !=  "") { echo 'has-error';} ?>">
		        ...
		        <select id='input_annee'  name="type" class="form-control" style="width: 184px;">
			        <option value="none">Choose the Type</option>
							<option value="course" <?php if (isset($_SESSION['type']) && $_SESSION['type'] == "course"){echo "selected";} ?>>Course Satisfaction</option>
							<option value="staff" <?php if (isset($_SESSION['type']) && $_SESSION['type'] == "staff"){echo "selected";} ?>>Staff Satisfaction</option>
							<option value="faculty" <?php if (isset($_SESSION['type']) && $_SESSION['type'] == "faculty"){echo "selected";} ?>>Faculty Satisfaction</option>
							<option value="alumni" <?php if (isset($_SESSION['type']) && $_SESSION['type'] == "alumni"){echo "selected";} ?>>Alumni Satisfaction</option>
		        </select>
			...
			<div style="margin-right: 175px;" class="form-group <?php if (isset($_SESSION['message_year']) && $_SESSION['message_year'] !=  "") { echo 'has-error';} ?>">
		        ...
		        <select id='input_annee'  name="year" class="form-control" style="width: 184px;">
