<?php if (isset($_GET['result_employer_t'])): ?>
	<?php
  ...
  $porcentage_y = array(
	...
	$sql_etu="SELECT distinct(em.result_id),em.result_id,em.email,`nom`,`prenom` FROM `tbl_etudiant` as e , resultat_alumni as r , Employer as em, result_employer as rm  WHERE $where $where_relation $where_result";
	//var_dump($sql_etu);
	...
	<table class="table table-bordered">
		...
		<thead>
			<tr>
				<form action="?result_employer_t" method="POST">
				<th style="width:50%">
		...
		<thead>
			<tr>
				<?php
					...
					$sql_count_employer = "SELECT count(DISTINCT(em.result_id)) as nb from `tbl_etudiant` as e , resultat_alumni as r , Employer as em, result_employer as rm where $where $where_relation";
					...
					$sql_count_employer = "SELECT count(DISTINCT(em.result_id)) as nb from `tbl_etudiant` as e , resultat_alumni as r , Employer as em, result_employer as rm where $where $where_relation $where_result";
					$req_count_employer = @mysql_query($sql_count_employer);
					$row_count_employer = @mysql_fetch_assoc($req_count_employer);
				...
				?>
				<th>Employer Response : <?php echo $nb_employer ?> out of <?php echo $ttl_employer; ?> | <?php echo round(($nb_employer/$ttl_employer)*100,2); ?>% response rate</th>
        <th><?php echo $_POST['annee'] ?></th>
      </tr>
		</thead>
		<thead>
			<th colspan="3">
            ...
            $year = $_POST['annee'];
                ...
                //var_dump($child_count);
                      ...
                      //var_dump($porcentage_y);
                      $porcentage_quest = $porcentage_y[$year][$parent_question_id];
                      //var_dump($porcentage_quest);
                      if ($porcentage_quest !=  "") {
                        $porcentage = $porcentage_quest;
                      }
								 ?>
                      ...
                      $porcentage_quest = $porcentage_y[$year][$question_kid_id];
                      //var_dump($porcentage_quest);
                      if ($porcentage_quest !=  "") {
                        $porcentage = $porcentage_quest;
                      }
                      ...
                      $type = "choix";
								 ?>
								 <?php if ($question != "Other (please specify)"){ ?>
										 <tr>
