...
<?php if (isset($_GET['surveys_results_mba']) && isset($_GET['id'])): ?>
    ...
    <?php if (isset($_GET['code_prof']) && isset($_GET['annee']) && isset($_GET['periode'])){ ?>
      ...
      $sql_cour_affectation= "SELECT a.* FROM `tbl_sondage_affectation` as a , `tbl_aff_etudiant` as e , `tbl_etudiant_deac` as et  WHERE a.`groupe` = 3 and a.`niveau` = \"mba\" and a.`code_prof` = $code_prof  and a.`annee` = \"$annee_per\" and a.`periode` = \"$periode\" AND a.`archive` = 0 and a.`id` = e.`id_affectation` and a.`type` = 1 and e.`code_inscription` = et.`code_inscription` and a.code_cours not in('SEM_555','SEM555','SEM 555') group by a.id";
...
<?php }elseif (isset($_GET['annee']) && isset($_GET['periode'])) { ?>
  ...
  <div class="row">
    <div class=" col-sm-offset-2 col-md-8">
      ...
      while ($row_prof_affectation = mysql_fetch_array($req_prof_affectation)){
        ...
        $row_prof = mysql_fetch_assoc($req_prof);
        $nom_prof = $row_prof['nom_prenom'];
        $sql_nb_cour = "SELECT COUNT(*) FROM `tbl_sondage_affectation` as a , `tbl_aff_etudiant` as e , `tbl_etudiant_deac` as et WHERE a.`groupe` = 3 and  a.`niveau` = \"mba\" and a.`annee` = $annee and a.`periode` = \"$periode\" AND a.`archive` = 0 AND a.`code_prof` = \"$code_prof\"and a.`id` = e.`id_affectation` and a.`type` = 1 and e.`code_inscription` = et.`code_inscription`";
  ...
  <?php }else{ ?>
    ...
    $sql_periode= "SELECT  `annee` , `periode` FROM  `tbl_sondage_affectation` WHERE `groupe` = 3 and `niveau` =  \"mba\" and `archive` = 0 and `type` = 1 GROUP BY  `annee` ,  `periode` order by `annee` DESC, `periode` ASC";
