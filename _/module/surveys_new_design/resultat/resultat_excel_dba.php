<?php if (isset($_GET['resultat_excel_dba']) && isset($_GET['id'])): ?>
	...
	<?php if (isset($_GET['code_prof']) && isset($_GET['annee']) && isset($_GET['periode'])){ ?>
		<?php
				...
				$sql_cour_affectation= "SELECT * FROM `tbl_sondage_affectation` WHERE `groupe` = 3 and `niveau` = \"dba\" and `code_prof` = $code_prof  and `annee` = \"$annee_per\" and `periode` = \"$periode\" AND `archive` = 0 ";
					...
					<div class="panel panel-default">
						<div class="panel-heading">
							<h4 class="panel-title text-info">
								<a style="font-size: 12px;" class="collapsed" href="?resultat_sondage_dba&id=<?php echo $id_sondage; ?>&affectation=<?php echo $id_affectation; ?>">
									<?php echo $code_cour." : "; ?>
	...
	<?php }elseif (isset($_GET['annee']) && isset($_GET['periode'])) { ?>
		<?php
			$periode = $_GET['periode'];
			$annee = $_GET['annee'];
			$sql_prof_affectation= "SELECT DISTINCT a.`code_prof` FROM `tbl_sondage_affectation` as a ,`tbl_professeur` as p , `tbl_aff_etudiant` as e , `tbl_etudiant_deac` as et WHERE a.`groupe` = 3 and a.`niveau` = \"dba\" and a.`annee` = $annee and a.`periode` = \"$periode\" AND a.`archive` = 0 and a.code_prof = p.code_prof and p.archive = 0 and a.`id` = e.`id_affectation` and a.`type` = 1 and e.`code_inscription` = et.`code_inscription`";
		...
		<h3 class="text-info">the number of students who responded to the survey <?php echo $nb_etudiant_r." / ".$nb_etudiant_tt; ?> </h3>
	        	...
	        	while ($row_prof_affectation = mysql_fetch_array($req_prof_affectation)){
	        		...
	        		$row_prof = mysql_fetch_assoc($req_prof);
	        		$nom_prof = $row_prof['nom_prenom'];
	        		$sql_nb_cour = "SELECT COUNT(*) FROM `tbl_sondage_affectation` WHERE `groupe` = 3 and  `niveau` = \"dba\" and `annee` = $annee and `periode` = \"$periode\" AND `archive` = 0 AND `code_prof` = \"$code_prof\"";
					$req_nb_cour = @mysql_query($sql_nb_cour);
					$row_nb_cour = mysql_fetch_array($req_nb_cour);
					$nb_cours = $row_nb_cour['0'];
					//////://///////
					$sql_new = "SELECT r.`id_affectation` , r.`id_reponse` , rep.`porcentage` as porcentage,count(r.`id_reponse`) as nb_porcentage FROM `tbl_sondage_affectation` as a , `tbl_resultat_sondage` as r , `tbl_reponse` as rep , `tbl_aff_etudiant` as e , `tbl_etudiant_deac` as et WHERE a.`groupe` = 3 and  a.`niveau` = \"dba\" and a.`annee` = $annee and a.`periode` = \"$periode\" AND a.`code_prof` = \"$code_prof\" and a.`id` = r.`id_affectation` and r.`id_reponse` = rep.`id` and a.`id` = e.`id_affectation` and a.`type` = 1 and e.`code_inscription` = et.`code_inscription` group by rep.`porcentage`";
					//var_dump($sql_new);
					$req_new = @mysql_query($sql_new);

					$sql_cour_affectation= "SELECT * FROM `tbl_sondage_affectation` WHERE `groupe` = 3 and   `niveau` = \"dba\" and `annee` = $annee and `periode` = \"$periode\" AND `code_prof` = \"$code_prof\" ";
	...
	<?php }else{ ?>
		<?php
	        ...
	        $res_sondage=@mysql_query($sql_sondage) or die ('erreur de selection des sessions');
	        $row_sondage=mysql_fetch_assoc($res_sondage);
			$sql_periode= "SELECT  `annee` ,  `periode` FROM  `tbl_sondage_affectation` WHERE `groupe` = 3 and `niveau` =  \"dba\" and `archive` = 0 GROUP BY  `annee` ,  `periode` ";
	    	...
	    	while ($row_periode = mysql_fetch_array($req_periode)) {
	    		...
	    		$sql_nb_surveys = "SELECT count(*) FROM `tbl_sondage_affectation` as a , `tbl_aff_etudiant` as e WHERE a.`niveau` = \"dba\" and a.`annee` = $annee and a.`periode` = \"$periode\"  and a.`id` = e.`id_affectation` and `groupe` = 3";
	    		...
	    		$sql_nb_surveys_r = "SELECT e.`id_affectation` FROM `tbl_sondage_affectation` as a , `tbl_resultat_sondage` as e WHERE a.`niveau` = \"dba\" and a.`annee` = $annee and a.`periode` = \"$periode\"  and a.`id` = e.`id_affectation` and `groupe` = 3 GROUP BY e.`id_affectation` , e.`code_etudiant`";
	    		...
	    		// les satisfactions
	    			$sql_cour_affectation= "SELECT a.* FROM `tbl_sondage_affectation` as a , `tbl_aff_etudiant` as e WHERE a.`niveau` = \"dba\" and a.`annee` = $annee and a.`periode` = \"$periode\"  and a.`id` = e.`id_affectation` and `groupe` = 3";
	        		$req_cour_affectation = @mysql_query($sql_cour_affectation);
	        		$ttl_etudiant = 0;
	        		$ttl_very_sat = 0;
					$ttl_sat = 0;
					...
					$sql_new = "SELECT r.`id_affectation` , r.`id_reponse` , rep.`porcentage` as porcentage,count(r.`id_reponse`) as nb_porcentage FROM `tbl_sondage_affectation` as a , `tbl_resultat_sondage` as r , `tbl_reponse` as rep WHERE `groupe` = 3 and   `niveau` = \"dba\" and `annee` = $annee and `periode` = \"$periode\" and a.`id` = r.`id_affectation` and r.`id_reponse` = rep.`id`  group by rep.`porcentage`";
					//var_dump($sql_new);
					$req_new = @mysql_query($sql_new);
							...
							<thead>
								<tr >
									<th colspan=2 style="text-align: left;color: black;font-weight: bold;">
										...
										<a href="Surveys.php?resultat_sondage_dba&annee=<?php echo $annee; ?>&periode=<?php echo $periode; ?>&id=<?php echo $id_sondage; ?>" class="btn btn-success" style="float: right;">
											Surveys by Faculty <i class="entypo-check"></i>
										</a>
										<a href="Surveys.php?resultat_sondage_dba&annee=<?php echo $annee; ?>&periode=<?php echo $periode; ?>&id=<?php echo $id_sondage; ?>"  style="color: #fff;" class="btn btn-success btn-xs">Export to excel</a>
									</th>
								</tr>
