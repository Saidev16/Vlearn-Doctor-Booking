		...
		<div class="card-header" style="background-color: #fff !important;">
          ...
          <?php if (isset($_GET['surveys_results_mba']) && isset($_GET['id'])): ?>
              ...
              <?php if (isset($_GET['code_prof']) && isset($_GET['annee']) && isset($_GET['periode'])){ ?>
                ...
                $sql_cour_affectation= "SELECT a.* FROM `tbl_sondage_affectation` as a , `tbl_aff_etudiant` as e   WHERE a.`groupe` = 3 and a.`niveau` = \"mba\" and a.`code_prof` = $code_prof  and a.`annee` = \"$annee_per\" and a.`periode` = \"$periode\" AND a.`archive` = 0 and a.`type` = 1 and  a.code_cours not in('SEM_555','SEM555','SEM 555') group by a.id";
                ...
                $sql_nb_etudiant_r = "SELECT count(distinct r.`code_etudiant`) FROM `tbl_sondage_affectation` as a ,`tbl_resultat_sondage` as r
                   WHERE a.`annee` = \"$annee_per\" and a.`periode` = \"$periode\" and a.`id` = r.`id_affectation`  and a.`type` = 1 ";
                ...
                ?>
                <h4 class="card-title">MBA Session: <?php  echo $periode.' '.$annee_per; ?></h4>
                <h6 class="card-subtitle"><?php echo ucwords($nom_prof); ?></h6>
                <div class="row">
                  <div class="col-md-12">
                    <?php
                    ...
                    while ($row_cour_affectation = mysql_fetch_array($req_cour_affectation)){
                      ...
                      $sql_nb_etudiant = "SELECT  COUNT( Distinct `code_etudiant`) FROM `tbl_resultat_sondage` as r
                        WHERE   r.`id_affectation` = \"$id_affectation\" AND r.`id_sondage` = \"$id_sondage\" ";
                      ...
                      <div class="col-lg-12">
                    		<div class="card-header" style="background-color: #fff !important;">
                              ...
                              <h4 class="panel-title text-info">
                                <a style="font-size: 12px;" class="collapsed" href="?item=<?php echo $_GET['item']; ?>&surveys_results_mba&id=<?php echo $id_sondage; ?>&affectation=<?php echo $id_affectation; ?>">
                                <?php echo $code_cour." : "; ?>
                        ...
                        <div class="card ">
                          <div class="card-body">
                              ...
                              $nb_q_ver = 0;
                              $sql_resultat = "SELECT Distinct `code_etudiant` FROM `tbl_resultat_sondage` as r    WHERE   r.`id_affectation` = \"$id_affectation\" AND r.`id_sondage` = \"$id_sondage\" ";
                              ...
                              $sql_new = "SELECT r.`id_affectation` , r.`id_reponse` , rep.`porcentage` as porcentage,count(r.`id_reponse`) as nb_porcentage FROM `tbl_sondage_affectation` as a
                              , `tbl_resultat_sondage` as r , `tbl_reponse` as rep
                              WHERE  a.`id` = r.`id_affectation` and r.`id_reponse` = rep.`id` and a.`id` = $id_affectation group by rep.`porcentage`";
                              //var_dump($sql_new);
                              ...
                              $satisfaction = 0;
                              $porcentage1 = 0;
                              $sql_new = "SELECT count(distinct(r.code_etudiant)) as nb ,`id_question`,r.code_etudiant, r.`id_affectation` , r.`id_reponse` , rep.`porcentage` as porcentage FROM `tbl_sondage_affectation` as a , `tbl_resultat_sondage` as r , `tbl_reponse` as rep
                                WHERE  a.`id` = r.`id_affectation` and r.`id_reponse` = rep.`id` and a.`id` = $id_affectation and  r.id_question in (9,22,21,19,18,17,13) group by rep.`porcentage`,`code_etudiant`,`id_question`";
                              //var_dump($sql_new);
                              $req_new = @mysql_query($sql_new);
                              while ($row_new = mysql_fetch_array($req_new)) {
                              ...
                              // calcule
                              $sql_new = "SELECT count(distinct(r.code_etudiant)) as nb ,r.code_etudiant,r.`id_question`, r.`id_affectation` , r.`id_reponse` , rep.`porcentage` as porcentage
                              FROM `tbl_sondage_affectation` as a , `tbl_resultat_sondage` as r , `tbl_reponse` as rep  WHERE  a.`id` = r.`id_affectation` and r.`id_reponse` = rep.`id`
                              and a.`id` = $id_affectation and  r.id_question in (1,2,3,4,5,6,7,8,15,16,23,24,25,26,27,31) group by rep.`porcentage` ,
                              r.`id_question`, r.`code_etudiant` order by `id_question`
                              	";
                              //var_dump($sql_new);
                              $req_new = @mysql_query($sql_new);
                              $satisfaction = 0;
                              ...
                              $porcentage3 = 0;
                              $sql_new = "SELECT count(distinct(r.code_etudiant)) as nb ,`id_question`,r.code_etudiant, r.`id_affectation` , r.`id_reponse` , rep.`porcentage` as porcentage FROM `tbl_sondage_affectation` as a , `tbl_resultat_sondage` as r , `tbl_reponse` as rep
                              WHERE  a.`id` = r.`id_affectation` and r.`id_reponse` = rep.`id` and a.`id` = $id_affectation and  r.id_question in (10,29,28) group by rep.`porcentage`,`code_etudiant`,`id_question`";
                              //var_dump($sql_new);
                              $req_new = @mysql_query($sql_new);
                              while ($row_new = mysql_fetch_array($req_new)) {
                                $satisfaction = $satisfaction + $row_new['porcentage'];
                              ...
                              $porcentage4 = 0;
                              $sql_new = "SELECT count(distinct(r.code_etudiant)) as nb ,`id_question`,r.code_etudiant, r.`id_affectation` , r.`id_reponse` , rep.`porcentage` as porcentage FROM `tbl_sondage_affectation` as a , `tbl_resultat_sondage` as r , `tbl_reponse` as rep
                              WHERE  a.`id` = r.`id_affectation` and r.`id_reponse` = rep.`id` and a.`id` = $id_affectation and  r.id_question in (12,14,10) group by rep.`porcentage`,`code_etudiant`,`id_question`";
                              //var_dump($sql_new);
                              $req_new = @mysql_query($sql_new);
                              while ($row_new = mysql_fetch_array($req_new)) {
                                $satisfaction = $satisfaction + $row_new['porcentage'];
                              ...
                              $porcentage5 = 0;
                              $sql_new = "SELECT count(distinct(r.code_etudiant)) as nb ,`id_question`,r.code_etudiant, r.`id_affectation` , r.`id_reponse` , rep.`porcentage` as porcentage FROM `tbl_sondage_affectation` as a , `tbl_resultat_sondage` as r , `tbl_reponse` as rep
                              WHERE  a.`id` = r.`id_affectation` and r.`id_reponse` = rep.`id` and a.`id` = $id_affectation and  r.id_question in (11,32,30) group by rep.`porcentage`,`code_etudiant`,`id_question`";
                              //var_dump($sql_new);
                              $req_new = @mysql_query($sql_new);
                              while ($row_new = mysql_fetch_array($req_new)) {
                                $satisfaction = $satisfaction + $row_new['porcentage'];
          ...
          <?php }elseif (isset($_GET['affectation'])){ ?>
            ...
            $sql_etudiant_inscrit = "SELECT count(distinct code_etudiant) FROM `tbl_resultat_sondage` as r , `tbl_etudiant` as et   WHERE r.`id_affectation` = $id_affectation ";
            ...
            while ($row_question = mysql_fetch_array($req_question)) {
              ...
              $sql_question_existe= "SELECT count(*) FROM `tbl_resultat_sondage` as r , `tbl_etudiant` as et  WHERE r.`id_affectation` = $id_affectation AND `id_sondage` = $id_sondage AND
                      ...
                      <tbody>
                        ...
                        while ($row_reponse = mysql_fetch_array($req_reponse)) {  ?>
                          <tr>
                            ...
                            $sql_nb_etudiants = "SELECT count(distinct r.`code_etudiant`) FROM `tbl_resultat_sondage` as r , `tbl_etudiant` as et WHERE r.`id_affectation` = $id_affectation AND `id_sondage` = $id_sondage AND `id_question` = $id_question AND `id_reponse` = $id_reponse";
          ...
          <?php }elseif (isset($_GET['annee']) && isset($_GET['periode'])) { ?>
            ...
            $periode = $_GET['periode'];
            $annee = $_GET['annee'];
            $sql_prof_affectation= "SELECT DISTINCT a.`code_prof` FROM `tbl_sondage_affectation` as a ,`tbl_professeur` as p  , `tbl_etudiant` as et   WHERE a.`groupe` = 3 and a.`niveau` = \"mba\" and a.`annee` = $annee and `periode` = \"$periode\"
            AND a.`archive` = 0 and a.code_prof = p.code_prof and p.archive = 0 and a.`type` = 1  ";
            //var_dump($sql_prof_affectation);
            $req_prof_affectation = @mysql_query($sql_prof_affectation);
            // les etudiants

            $sql_nb_etudiant_r = "SELECT count(distinct r.`code_etudiant`) FROM `tbl_sondage_affectation` as a ,`tbl_resultat_sondage` as r  ,`tbl_professeur` as p  , `tbl_etudiant` as et  WHERE a.`annee` = \"$annee\" and a.`periode` = \"$periode\" and a.`id` = r.`id_affectation` and a.`groupe` = 3 and a.code_prof = p.code_prof and p.archive = 0  and a.`type` = 1 ";
            //var_dump($sql_nb_etudiant_r);
            $req_nb_etudiant_r = @mysql_query($sql_nb_etudiant_r);
            $row_nb_etudiant_r = mysql_fetch_array($req_nb_etudiant_r);
            $sql_nb_etudiant_tt = "SELECT count(*) FROM `tbl_sondage_affectation` as a  ,`tbl_professeur` as p , `tbl_etudiant` as et  WHERE a.`niveau` = \"mba\" and a.`annee` = $annee and a.`periode` = \"$periode\"  and a.`id` = e.`id_affectation` and a.`groupe` = 3 and a.code_prof = p.code_prof and p.archive = 0  and a.`type` = 1 ";
            ...
            <div class="row">
                ...
                while ($row_prof_affectation = mysql_fetch_array($req_prof_affectation)){
                  ...
                  $row_prof = mysql_fetch_assoc($req_prof);
                  $nom_prof = $row_prof['nom_prenom'];
                  $sql_nb_cour = "SELECT COUNT(distinct(a.id)) FROM `tbl_sondage_affectation` as a  WHERE a.`groupe` = 3 and  a.`niveau` = \"mba\" and a.`annee` = $annee and a.`periode` = \"$periode\"
                  AND a.`archive` = 0 AND a.`code_prof` = \"$code_prof\"  and a.`type` = 1  and a.code_cours not in ('SEM_555','SEM 555')";
                  ...
                  r.id_question in (9,22,21,19,18,17,13)
                  ...
                  r.id_affectation in (SELECT distinct(a.id) FROM `tbl_sondage_affectation` as a WHERE a.`groupe` = 3 and  a.`niveau` = \"mba\" and a.`annee` = $annee and a.`periode` = \"$periode\" AND a.`archive` = 0 AND a.`code_prof` = \"$code_prof\" and a.`type` = 1 and a.code_cours not in ('SEM_555','SEM 555')) and r.`id_reponse` = rep.`id`and
                  r.id_question in (1,2,3,4,5,6,7,8,15,16,23,24,25,26,27,31)
                  group by rep.`porcentage`,`code_etudiant`,`id_question`";
                  ...
                  <div class="col-lg-6">
                		<div class="card-header" style="background-color: #fff !important;">
                			<div class="row show-grid">
                				<div class="col-md-12 mt-2">
                          <a class="collapsed" href="?item=<?php echo $_GET['item']; ?>&surveys_results_mba&id=<?php echo $id_sondage; ?>&code_prof=<?php echo $code_prof; ?>&periode=<?php echo $periode; ?>&annee=<?php echo $annee; ?>">
                            <?php echo ucwords($nom_prof); ?>
                            <span class="btn waves-effect waves-light btn-sm btn-success" style="float: right;"><?php echo $nb_cours; if ($nb_cours == 1) {echo "  course";}else{echo "  courses";} ?> <i class="entypo-plus-circled"></i></span>
                          </a>
            ...
            <?php }else{ ?>
              ...
              while ($row_periode = mysql_fetch_array($req_periode)) {
                ...
                // Q1 --> Q8
                // nb etudiants
                $sql_new = "SELECT count(distinct(r.code_etudiant)) as nb FROM `tbl_sondage_affectation` as a , `tbl_resultat_sondage` as r , `tbl_reponse` as rep , `tbl_etudiant` as et  WHERE  a.`id` = r.`id_affectation` and r.`id_reponse` = rep.`id` and a.`id` = $id_affectation and e.`code_inscription` = et.`code_inscription` and r.id_question in (1,2,3,4,5,6,7,8,15,16,23,24,25,26,27,31)
                ";
                //var_dump($sql_new);
                $req_new = @mysql_query($sql_new);
                $row_new = mysql_fetch_array($req_new);
                ...
                a.`type` = 1  group by rep.`porcentage`,`code_etudiant`,`id_question`";
                //var_dump($sql_new);
                $req_new = @mysql_query($sql_new);
                $porcentage4 = 0;
                $satisfaction = 0;
                  ...
                  ?>
                	...
                	<div class="col-lg-6">
                		<div class="card-header" style="background-color: #fff !important;">
                          ...
                          <a href="?item=<?php echo $_GET['item']; ?>&resultat_by_prof_mba&annee=<?php echo $annee; ?>&periode=<?php echo $periode; ?>&id=<?php echo $id_sondage; ?>"  style="color: #fff;" class="btn btn-success btn-xs">Export to excel</a>
                				</div>
                			</div>
                		</div>
                    <div class="card ">
