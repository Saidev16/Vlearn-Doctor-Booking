...
if (isset($_POST['sexe'])){
  $_SESSION['error'] = 0;
  $message = 0;
  $prenom=addslashes(trim($_POST['sexe']));
  $_SESSION['sexe'] = $sexe;
  if ($sexe == "") {
    $message = 1;
  }
...
if (isset($_POST['nationalite'])){
  $_SESSION['error'] = 0;
  $message = 0;
  $nationalite=addslashes(trim($_POST['nationalite']));
...
if (isset($_POST['lieu_naissance'])){
  $_SESSION['error'] = 0;
  $message = 0;
  $lieu_naissance=addslashes(trim($_POST['lieu_naissance']));
  $_SESSION['lieu_naissance'] = $lieu_naissance;
  if ($lieu_naissance == "") {
    $message = 1;
  }
}

if (isset($_POST['code_bac'])){
  $_SESSION['error'] = 0;
  $message = 0;
  $code_bac=addslashes(trim($_POST['code_bac']));
  $_SESSION['code_bac'] = $code_bac;
  if ($code_bac == "") {
    $message = 1;
  }
}

if (isset($_POST['english_bac'])){
  $_SESSION['error'] = 0;
  $message = 0;
  $english_bac=addslashes(trim($_POST['english_bac']));
  $_SESSION['english_bac'] = $english_bac;
  if ($english_bac == "") {
    $message = 1;
  }
}
...
if (isset($_POST['groupe'])){
  $_SESSION['error'] = 0;
  $message = 0;
  $groupe=addslashes(trim($_POST['groupe']));
...
if (isset($_POST['parentname'])){
  $_SESSION['error'] = 0;
  $message = 0;
  $parentname=addslashes(trim($_POST['parentname']));
...
if (isset($_POST['parentemail'])){
  $_SESSION['error'] = 0;
  $message = 0;
  $parentemail=addslashes(trim($_POST['parentemail']));
...
if (isset($_POST['parentphone'])){
  $_SESSION['error'] = 0;
  $message = 0;
  $parentphone=addslashes(trim($_POST['parentphone']));
  $_SESSION['parentphone'] = $parentphone;
  if ($parentphone == "") {
    $message = 1;
  }
}
...
if ($message == 0 ) {
 $sqlzin="SELECT max(`code_inscription`) as max FROM tbl_etudiant_marbella";
$sqlzin1 =mysql_query($sqlzin) or die("erreur lors de traitement de F*");

$ab= mysql_fetch_assoc($sqlzin1);
$cu= $ab["max"];
$code_inscription= $cu+1;
  $sql_insert="INSERT INTO tbl_etudiant_marbella (`prefixe` ,code_inscription,`nom` , `prenom` , `date_naissance` , `nationalite` , `adresse` , `sexe` , `code_bac` , `serie_bac` ,
...
VALUES ('MARB','$code_inscription','$nom', '$prenom', '$date_naissance', '$nationalite', '$adresse', '$sexe', '$code_bac', '$serie_bac', '$date_insc', '$tel', '$mail', '$annee', '$idSession', '$filiere', '$lieu_naissance', '$cin', '$cinq_photo', '$original_bac', '$copie_bac', '$english_bac', '$trois_lettre', '$trois_enveloppe', '$buletin_note',  '$reglement', '$copie_cin', '$extrait', '$note_etudesoc', '$note_fr', '$note_sciences', '$note_math', '$aquise', '$nom', '$pass', 1, '$groupe', '$activite', '$annee_inscription', '$niveau', '$ville', '$elearning', '$piimt', '$aul', '$umt', '$type','$parentname', '$parentphone', '$parentemail', '$parentname', '$pass');";
...
/*$sqlfinance="insert into tbl_finance (`prefixe` ,code_inscription,frais_etude) values('MOR','$code_inscription',$tuition)";
 ...
 $sqlfinance="insert into tbl_finance_marbella (`prefixe` ,code_inscription) values('MARB','$code_inscription')";
 @mysql_query($sqlfinance)or die ("Error save finance");
          ...
          if ($type !='Prospect') { ?>
           ...
           ?>
 			...
 			<!--
                       window.location.replace('/module/api_testing/create_user.php?code_inscription=<?php echo $code_inscription; ?>&prefixe=MARB');
 			//-->
 			</script>
               <?php
                   }
      ...
      ?>
        <script type="text/javascript" language="JavaScript1.2">
            <!--
            window.location.replace('gestion_des_etudiants_marbella.php');
            //-->
        </script>
      <?php
    }
	...
	<td width="20%">
	  <table border="0" align="right" width="0%" cellpadding="10" cellspacing="4" id="link" >
	    <tr>
		  ...
		  <td valign="top" align="center">
		   <a href="gestion_des_etudiants_marbella.php"><div class="cancel"></div>Cancel</a>
		  </td>
		</tr>
	  </table>
	</td>
...
<style media="screen">
    ...
    <?php if (isset($_SESSION['error']) && $_SESSION['error'] ==  "1"): ?>
      ...
      <div style="width: 96%;" class="alert alert-danger" role="alert">
        <p class="close" style="float: right;cursor:pointer;">X</p>
  ...
  <form method="post" action="gestion_des_etudiants_marbella.php?new=oui" name="f_ajout" >
         ...
         <td valign="top" width="100%"><table border="0" cellpadding="0" cellspacing="2" width="100%" class="cellule_table">
		  ...
		  <!-- <tr>
           ...
           <tr>
             ...
             <td><input id="prenom" onkeyup="correct('#prenom')" type="text" name="prenom" id="prenom" class="input" value="<?php
                      if (isset($_SESSION['error']) && $_SESSION['error'] == 1) {
                          echo $_SESSION['prenom'];
                      } ?>" <?php if ($_SESSION['error'] == 1 && isset($_SESSION['error']) && $_SESSION['prenom'] == "") { echo "style='border: ridge red 1px;'";  } ?>/>
              </td>
             <td></td>

             <td><label for="nom">Last Name  :</label>             </td>
             
              <td><input id="nom" onkeyup="correct('#nom')" type="text" name="nom" id="nom" class="input" value="<?php
                      if (isset($_SESSION['error']) && $_SESSION['error'] == 1) {
                          echo $_SESSION['nom'];
                      } ?>" <?php if ($_SESSION['error'] == 1 && isset($_SESSION['error']) && $_SESSION['nom'] == "") { echo "style='border: ridge red 1px;'";  } ?>/>
              </td>
           </tr>
           <tr>
           ...
           <tr>
             <td><label for="adresse">Adress :</label>             </td>
              ...
              <td><input id="adresse" onkeyup="correct('#adresse')" type="text" name="adresse" id="adresse" class="input" value="<?php
                      if (isset($_SESSION['error']) && $_SESSION['error'] == 1) {
                          echo $_SESSION['adresse'];
                      } ?>" <?php if ($_SESSION['error'] == 1 && isset($_SESSION['error']) && $_SESSION['adresse'] == "") { echo "style='border: ridge red 1px;'";  } ?>/>
              </td>


             <td></td>
             <td><label for="tel">Cell :</label>             </td>
           <td><input id="tel" onkeyup="correct('#tel')" type="text" name="tel" id="tel" class="input" value="<?php
                      if (isset($_SESSION['error']) && $_SESSION['error'] == 1) {
                          echo $_SESSION['tel'];
                      } ?>" <?php if ($_SESSION['error'] == 1 && isset($_SESSION['error']) && $_SESSION['tel'] == "") { echo "style='border: ridge red 1px;'";  } ?>/>
              </td>
           ...
           <tr>
            ...
            <td><input id="cin" onkeyup="correct('#cin')" type="text" name="cin" id="cin" class="input" value="<?php
                      if (isset($_SESSION['error']) && $_SESSION['error'] == 1) {
                          echo $_SESSION['cin'];
                      } ?>" <?php if ($_SESSION['error'] == 1 && isset($_SESSION['error']) && $_SESSION['cin'] == "") { echo "style='border: ridge red 1px;'";  } ?>/>
              </td>
             <td></td>
		  ...
		  ?>
           ...
           <tr>
             ...
             <td><input id="mail" onkeyup="correct('#mail')" type="text" name="mail" id="mail" class="input" value="<?php
                      if (isset($_SESSION['error']) && $_SESSION['error'] == 1) {
                          echo $_SESSION['mail'];
                      } ?>" <?php if ($_SESSION['error'] == 1 && isset($_SESSION['error']) && $_SESSION['mail'] == "") { echo "style='border: ridge red 1px;'";  } ?>/>
              </td>

             <td></td>
             <td><label for="nationalite">Nationality :</label>             </td>
               <td><input id="nationalite" onkeyup="correct('#nationalite')" type="text" name="nationalite" id="nationalite" class="input" value="<?php
                      if (isset($_SESSION['error']) && $_SESSION['error'] == 1) {
                          echo $_SESSION['nationalite'];
                      } ?>" <?php if ($_SESSION['error'] == 1 && isset($_SESSION['error']) && $_SESSION['nationalite'] == "") { echo "style='border: ridge red 1px;'";  } ?>/>
              </td>
           </tr>
           <tr>
             <td colspan="4" height="3px"></td>
           ...
           <tr>
             <td><label for="lieu_naissance">Name of Lasted School attented: </label></td>
              <td><input id="lieu_naissance" onkeyup="correct('#lieu_naissance')" type="text" name="lieu_naissance" id="lieu_naissance" class="input" value="<?php
                      if (isset($_SESSION['error']) && $_SESSION['error'] == 1) {
                          echo $_SESSION['lieu_naissance'];
                      } ?>" <?php if ($_SESSION['error'] == 1 && isset($_SESSION['error']) && $_SESSION['lieu_naissance'] == "") { echo "style='border: ridge red 1px;'";  } ?>/>
              </td>
             <td></td>
             <td><label for="code_bac">School Adress  :</label>             </td>
              <td><input id="code_bac" onkeyup="correct('#code_bac')" type="text" name="code_bac" id="code_bac" class="input" value="<?php
                      if (isset($_SESSION['error']) && $_SESSION['error'] == 1) {
                          echo $_SESSION['code_bac'];
                      } ?>" <?php if ($_SESSION['error'] == 1 && isset($_SESSION['error']) && $_SESSION['code_bac'] == "") { echo "style='border: ridge red 1px;'";  } ?>/>
              </td>
           </tr>
           <tr>
             <td colspan="4" height="3px"></td>
...
<option value="Zimbabwe">Zimbabwe</option>
        ...
        <td><label for="type ">Type of student: </label></td>
          ...
          <tr>
             ...
             <td><input id="parentname" onkeyup="correct('#parentname')" type="text" name="parentname" id="parentname" class="input" value="<?php
                      if (isset($_SESSION['error']) && $_SESSION['error'] == 1) {
                          echo $_SESSION['parentname'];
                      } ?>" <?php if ($_SESSION['error'] == 1 && isset($_SESSION['error']) && $_SESSION['parentname'] == "") { echo "style='border: ridge red 1px;'";  } ?>/>
              </td>
             <td></td>
             <td><label for="parentphone">Parent Phone :</label>             </td>
             <td><input id="parentphone" onkeyup="correct('#parentphone')" type="text" name="parentphone" id="parentphone" class="input" value="<?php
                      if (isset($_SESSION['error']) && $_SESSION['error'] == 1) {
                          echo $_SESSION['parentphone'];
                      } ?>" <?php if ($_SESSION['error'] == 1 && isset($_SESSION['error']) && $_SESSION['parentphone'] == "") { echo "style='border: ridge red 1px;'";  } ?>/>
              </td>
           </tr>
           <tr>
             <td colspan="4" height="3px"></td>
           ...
           <tr>
             ...
             <td><input id="parentemail" onkeyup="correct('#parentemail')" type="text" name="parentemail" id="parentemail" class="input" value="<?php
                      if (isset($_SESSION['error']) && $_SESSION['error'] == 1) {
                          echo $_SESSION['parentemail'];
                      } ?>" <?php if ($_SESSION['error'] == 1 && isset($_SESSION['error']) && $_SESSION['parentemail'] == "") { echo "style='border: ridge red 1px;'";  } ?>/>
              </td>
           <tr>
             <td colspan="4" height="3px"></td>
           </tr>
