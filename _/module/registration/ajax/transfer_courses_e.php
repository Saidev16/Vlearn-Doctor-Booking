...
if ($_POST != array()) {
  ...
  if ($nbr == 0 or $method == "edit") {
    ...
    if ($method == "add") {
      ...
      value('$code_inscription', '$code_cours','$code_cours_testing', '$idSession', 0, '$section','T',$grade)";
      //var_dump($sql);
      @mysql_query($sql) or die ('erreur lors de creation de la fiche  de notes');
    }
    if ($method == "edit") {
      $sql = "UPDATE $table_note_sql SET `idSession`= $idSession , letter_grade = 'T' WHERE `code_inscription` = '$code_inscription'
      and `code_cours` = $code_cours";
      @mysql_query($sql) or die ('erreur lors de la modification de la note');
    }
