...
<?php
		...
		$transcript[6] = $_POST['code_cours7'];
		...
		$sql = "SELECT COUNT(*) AS nbr FROM tbl_inscription_cours_all WHERE code_inscription='$code_inscription' 
		...
		if ($nbr==0){
			...
			$sql = "INSERT INTO tbl_inscription_cours_all (`code_cours`, `code_inscription`, code_cours_testing, `date_inscription`, `idSession`)
			VALUES ('$code_cours', '$code_inscription','$code_cours_testing', '$date', '$idSession');";
 	         ...
 	         @mysql_query($sql)or die ('Erreur :: inscription11');
				...
				$sql = "insert into tbl_note_all(code_inscription, `code_cours`, code_cours_testing, idSession, archive, section,letter_grade) 
			...
			<!--	
 			window.location.replace('gestion_inscription_all.php?code_inscription=<?=$code_inscription?>&idSession=<?=$idSession?>');
			//-->
			</script>
			  <?php
 			  }
...
<table border="0" width="100%" cellpadding="0" cellspacing="0" align="center" class="haut_table">
  <tr>
    ...
    <td width="78%" class="titre">&nbsp; New registration  <span class="task">[Add]</span> 
    </td>
	<td width="22%">
	  <table border="0" align="right" width="0%" cellpadding="10" cellspacing="4" id="link" >
	    <tr>
		  ...
		  <td valign="top" align="center">
		   <a href="gestion_inscription_all.php" ><div class="cancel"></div>Cancel</a>
		  </td>
		</tr>
	  </table>
	</td> 
 ...
 <form method="post" action="gestion_inscription_all.php?full_addT=oui" name="f_ajout">
	 ...
	 <tr><td colspan="4" height="3px"></td></tr>
           <tr><td><label for="code_cours7">Course Title 7:</label> </td>
 		  <td colspan="3">
 		 <select name="code_cours7" id="code_cours7" class="input">
		 	<option value="">Select Course Title</option>
		  <?php 
			$sql5="SELECT  code_cours,  titre FROM $tbl_cours  GROUP BY  code_cours";
			$req=@mysql_query($sql5);
		  ...
		  <?php 
		   $sql6="select code_inscription, nom, prenom from tbl_etudiant_all where archive = 0 AND groupe not in (5,7) order by prenom, nom";
		...
		<option value="">Select Session</option>
		  ...
		  $cs=$row['idSession'];
		  	$cc=$row['annee_academique'];
		  	$ss=$row['session'];
		  ?>
		  <option value="<?=$cs?>" <?=($cs==$idSession) ? $selected : ''?>><?=$ss.$cc?></option>
		  <?php
		  }
		  ?>
		  </select>
