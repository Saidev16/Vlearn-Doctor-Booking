<?php
	...
	$sql="SELECT type_cours_id FROM tbl_inscription_cours_algeria
		...
		$type= $row['type_cours_id'];
		  ...
		  $sql="DELETE FROM tbl_inscription_cours_algeria  
		  ...
		  $sql="DELETE FROM tbl_note_algeria  
		  		  WHERE code_cours='$code_cours' 
				  AND code_inscription='$code_inscription'
				  AND idSession='$idSession'";
...
?>
        ...
        <!--
        	window.location.replace('gestion_inscription_algeria.php?code_cours=<?=$code_cours?>&idSesion=<?=$idSession?>&type=<?=$type?>&code=<?=$code_cours?>&code_inscription=<?=$code_inscription?>');
        //-->
        </script>