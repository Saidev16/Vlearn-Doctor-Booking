...
$id=$_POST['id'];
	$code_inscription=$_POST['code_inscription'];
		...
		$sql="UPDATE tbl_finance_paiement SET designation = '$designation', 
		...
		/*$sql="UPDATE tbl_finance_paiement_bakzin SET designation = '$designation', 
		type_paiement= '$type_paiement',
		date_paiement= '$date_paiement',
		recu= '$recu',
		somme= '$somme'
		...
		*/
		...
		$sql="select sum(somme) as nbre from tbl_finance_paiement  where code_inscription = '$code_inscription' AND prefixe = '$prefixe' ";
		$reqab =mysql_query($sql) or die("erreur lors de traitement de F*");
        $ab= mysql_fetch_assoc($reqab);
	     $payee=$ab["nbre"];
	
	
		$sql="UPDATE tbl_finance SET payee = '$payee'
 ...
 $table=tbl_etudiant_benin;
				 ...
				 $sql="SELECT e.nom, e.prenom ,e.prefixe, p.* 
						 FROM $table AS e, tbl_finance_paiement AS p
						 WHERE e.code_inscription = p.code_inscription AND p.id = $id1 and e.prefixe= '$prefixe'";
...
<table border="0" width="100%" cellpadding="0" cellspacing="0" align="center" class="haut_table">
 ...
 <form method="post" action="gestion_finance.php?modif_paiement=oui" name="f_ajout" >
       ...
       <tr>
         <td valign="top" width="100%">
		  ...
		  for ($i=1960; $i<=$param; $i++){*/
		   for ($i=date('Y')-2; $i>1996,$i<2020; $i++){?>
