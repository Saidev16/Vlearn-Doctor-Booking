...
-->
 ...
 $CGPA=$count=$credit=0;
	 ...
	 $sql="SELECT concat(nom, ' ', prenom) AS name, date_inscription, date_naissance 
	 FROM $tbl_etudiant WHERE code_inscription= '$code_inscription' LIMIT 1";
  ...
  <table style="width:100%" align="left" cellpadding="0" cellspacing="0">
    ...
    <tr>
        ...
        <td><span class="gras">Degree pursued: </span> </td>
    </tr>
    <tr>
    	<td><span class="gras">First registration: </span><?php echo $code_inscription;?> </td>
        <td><span class="gras">Major: </span> </td>
    </tr>
  </table>
  <br />
	...
	?>
    	<td style="width:50%" valign="top">
			...
			<?php
             ...
             if(($row['letter_grade']!= 'T' && $row['letter_grade']!= 'TC') && ($row['letter_grade']!='')){
             $j++;
             }
             $GPA += $row['gpa'];
             if($row['letter_grade']!= 'T' && $row['letter_grade']!= 'TC' && $row['letter_grade']!=''){ $credit+=$row['nbr_credit'];}
             if($row['letter_grade']== 'T' or $row['letter_grade']== 'TC')  $transfer+=1 ;
              if ($header == 0 ) {
            ?>
