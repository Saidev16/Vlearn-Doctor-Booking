...
-->
 ...
 $sql="SELECT e.*
	 ...
	 <tr>
		...
		?></td>
				<td><span class="gras">Degree pursued: </span> <?php echo 'BBA';?></td>
			</tr>
			<tr>
				<td><span class="gras">First registration: </span><?php $date=$row['date_inscription'];
		$tab=split('[/.-]',$date); 
		echo $tab[2].'-'.$tab[1].'-'.$tab[0];/*echo $row['date_inscription'];*/?> </td>
	...
	";
			...
			?>
			 ...
			 AND n.archive = 0";
					 ...
					 if(($row['letter_grade']!= 'T' && $row['letter_grade']!= 'TC') && ($row['letter_grade']!='') && $row['letter_grade']!= 'W'&& $row['letter_grade']!= 'X'){
					 $j++;
					 }
					 $GPA += $row['gpa'];
					 if($row['letter_grade']!= 'T' && $row['letter_grade']!= 'TC' && /*$row['letter_grade']!='' && */$row['letter_grade']!= 'W'&& $row['letter_grade']!= 'X'){ $credit+=$row['nbr_credit'];}
					...
					?>
					 ...
					 <tr>
						...
						<td style="width:65%" align="left"><?php echo trim(stripslashes($row["titre"]))/*echo stripslashes($row['titre']);*/;?></td>
  ...
  <tr>
    <td>University Credits: <?php echo $credit; ?></td>
  </tr>
  <tr>
    <td>Transfer Credits: <?php echo $transfer*3; ?></td>
  </tr>
  <tr>
    <td>Cumulative Grade Point Average (CGPA): <?php echo  $count > 0 ? round($CGPA/$count, 2) : '' ; ?></td>
  </tr>
  <tr>
